...
**Why restarts are usually unnecessary** (shapes a lot of behavior): Claude Code re-reads the plaintext credential file (`~/.claude/.credentials.json`) on change; on macOS the Keychain is read at session start (+ a 30s-cached storage layer) but **mid-session Keychain changes alone do NOT propagate** — measured 2026-07-31 with live sessions. That is why the active-credential write mirrors into an existing `.credentials.json` (dual-write) and active reads are freshest-wins by `expiresAt` (`credentials.py::_write_oauth_credentials` / `_read_credentials`): with the file kept current, a running session adopts a switch on its next API call (~10s). Never regress to keychain-only active writes when the file exists.
