...
class TestRefreshRateLimitBackoff:
    """A token-endpoint (refresh) 429 (oauth.TOKEN_RATE_LIMITED) is surfaced as
    USAGE_TOKEN_EXPIRED ("login expired") so the menu offers click-to-reauth — the
    only in-place recovery for a refresh-rate-limited backup (a fresh token
    sidesteps the shared per-IP limit, which won't reset while cswap keeps probing).
    The resulting DEAD health arms the per-account dead-reprobe backoff so cswap
    stops hammering the limit (window behavior covered by TestDeadReprobeBackoff)."""

    def test_token_rate_limited_renders_login_expired_and_arms_dead_reprobe(
        self, temp_home, monkeypatch
    ):
        s = _make_switcher()
        h = _Harness(s, monkeypatch)
        out = h.round({"2": _oauth.TOKEN_RATE_LIMITED})
        assert out[1] == USAGE_TOKEN_EXPIRED            # login-expired -> reauth item
        assert _oauth.account_headroom(out[1]) is None  # never auto-selected
        assert s._usage_health.get("2") == "DEAD"       # hooks the dead path
        assert "2" in s._usage_dead_until               # dead-reprobe backoff armed
    ...
    def test_refresh_429_stops_the_round(self, temp_home, monkeypatch):
        ...
        assert out[0] == USAGE_TOKEN_EXPIRED                    # slot 1 login-expired
